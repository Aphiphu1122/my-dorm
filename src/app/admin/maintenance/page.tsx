    ...
    <div className="flex min-h-screen bg-white">
      ...
      <div className="flex-1 p-8 max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6 px-4 md:px-6">
          <div>
            <h1 className="text-3xl font-bold text-[#0F3659]">รายการแจ้งซ่อม</h1>
            <h2 className="text-gray-400 mt-2">
              จัดการรายการแจ้งซ่อมทั้งหมดในระบบ
            </h2>
          </div>
        </div>
          ...
          <div className="overflow-x-auto rounded-lg shadow-md border border-gray-200 bg-white mx-4 md:mx-6">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-100">
                <tr>
                  {["วันที่แจ้ง", "รหัสคำร้อง", "ห้อง", "รายละเอียด", "สถานะ"].map((header) => (
                    <th
                      key={header}
              ...
              <tbody className="divide-y divide-gray-100">
                {requests.map((req) => (
                  <tr
                    ...
                    role="link"
                    aria-label={`ดูรายละเอียดคำร้องแจ้งซ่อม ${req.id} ห้อง ${req.room.roomNumber}`}
                  >
